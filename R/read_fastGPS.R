...
read_gps <- function(gps_file,to_lower = TRUE) {
  ...
  coltypes_list <- list(
    ...
    count = readr::col_number(),
    time_offset = readr::col_number(),
    locnumber = readr::col_number(),
    failures = readr::col_number(),
    hauled_out = readr::col_number(),
    satellites = readr::col_number(),
    initlat = readr::col_number(),
    initlon = readr::col_number(),
    inittime = readr::col_datetime(format = "%H:%M:%S %d-%b-%Y"),
    inittype = readr::col_character(),
    latitude = readr::col_number(),
    longitude = readr::col_number(),
    height = readr::col_number(),
    bad_sats = readr::col_character(),
    residual = readr::col_number(),
    time_error = readr::col_number()
  )
